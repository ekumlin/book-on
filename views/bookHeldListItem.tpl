...
?><div class="list-item book-held-list-item">
	...
	<div class="returnDate" title="<?php echo date(config('datetime-format'), $viewBag['bookCopy']->returnDate); ?>"><?php echo $viewBag['bookCopy']->returnDate ? date(config('date-format') . ' (' . config('time-format') . ')', $viewBag['bookCopy']->returnDate) : ''; ?></div>
